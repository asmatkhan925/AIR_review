...
|---|---|---|---|---|---|
| `08_tables/table_01_low_resource_asr_taxonomy.md` | Sections 3 and 6; also supports Section 12 | `block_g_cross_block_taxonomy_synthesis_matrix.csv`; `block_g_core_claim_synthesis_map.csv`; Sections 3, 6, and 12 drafts | Draft table ready for manuscript review | The table groups the 44 taxonomy rows into compact manuscript-facing categories; detailed row-level evidence remains in Block G. A table note gives compact representative anchors rather than row-level citations. | No watchlist-only evidence used for central claims |
| `08_tables/table_02_dataset_benchmark_comparison.md` | Section 5 | `dataset_benchmark_matrix.csv`; `core_60_reference_set.csv`; Section 5 draft | Draft table ready for manuscript review | The MLS row avoids unverified hours; the IndicSUPERB row should remain bounded to regional benchmark framing during final integration. Internal verification wording has been moved out of the manuscript-facing table. | Watchlist-only resources such as emerging 2025-2026 benchmarks are excluded from the central comparison |
| `08_tables/table_03_adaptation_strategy_decision_matrix.md` | Section 8 | `adaptation_strategy_matrix.csv`; `block_g_core_claim_synthesis_map.csv`; `block_g_research_gap_agenda_matrix.csv`; Section 8 draft | Draft decision matrix ready for manuscript review | LoRA/QLoRA entries use method anchors unless direct ASR evidence is separately verified; prompting/context rows are evaluation-risk oriented and should remain bounded. Exact compute depends on study-level reporting. | Watchlist PEFT/prompting evidence is bounded as emerging and not used as proof of central ASR effectiveness |
